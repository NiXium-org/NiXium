			...
			perSystem = { system, config, ... }: {
				...
				mission-control.scripts = {
					...
					"switch" = {
						...
						exec = ''
							...
							case "$*" in
								"all")
									for system in $(find ./src/nixos/machines/* -type d | sed "s#^./machines/##g" | tr '\n' ' '); do
										echo "Switching configuration for system: $system"

										${inputs.nixpkgs.legacyPackages.${system}.nixos-rebuild}/bin/nixos-rebuild switch --flake "$repoDir#$system" --target-host "root@$system.systems.nx" --option eval-cache false
									done
									;;
								"")
									hostname="$(hostname --short)"
									...
									${self.inputs.nixpkgs.legacyPackages.${system}.openssh}/bin/ssh root@localhost ${inputs.nixpkgs.legacyPackages.${system}.nixos-rebuild}/bin/nixos-rebuild switch --flake "$repoDir#$hostname" --option eval-cache false
									;;
								*)
					...
					"rekey" = {
						...
						exec = ''
							# NixOS
							EDITOR=vim RULES=./src/nixos/secrets.nix ragenix -r
						'';
					};
				};
