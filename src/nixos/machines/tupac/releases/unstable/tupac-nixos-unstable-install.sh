...
###! This script performs declarative installation of NiXium-Managed NixOS UNSTABLE for the TUPAC system
###!
###! For that we utilize:
...
if [ -s "$ragenixIdentity" ]; then
	...
	[ -s "/run/agenix/tupac-disks-password" ] || age --identity "$ragenixIdentity" --decrypt --output "/run/agenix/tupac-disks-password" "$secretPasswordPath"

	[ -s "/run/agenix/tupac-ssh-ed25519-private" ] || age --identity "$ragenixIdentity" --decrypt --output "/run/agenix/tupac-ssh-ed25519-private" "$secretSSHHostKeyPath"
	...
	[ -s "/run/agenix/tupac-disks-password" ] || echo "000000" > "/run/agenix/tupac-disks-password"

	[ -s "/run/agenix/tupac-ssh-ed25519-private" ] || ssh-keygen -f "/run/agenix/tupac-ssh-ed25519-private" -N ""
fi

#! Pre-build the system configuration
status "Pre-building the system configuration"
nixos-rebuild build --flake "$FLAKE_ROOT#nixos-tupac-unstable" # pre-build the configuration

#! Perform the Payload
status "Performing the system installation"
disko-install \
	--flake "$FLAKE_ROOT#nixos-tupac-unstable" \
	--mode format \
	--debug \
