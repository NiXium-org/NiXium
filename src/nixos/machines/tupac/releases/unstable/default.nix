...
in {
	...
	perSystem = { system, pkgs, inputs', self', ... }: {
		...
		packages.nixos-tupac-unstable-unattended-installer-iso = inputs.nixos-generators.nixosGenerate {
			pkgs = import inputs.nixpkgs {
				inherit system;
			...
			modules = [
				{
					...
					systemd.services.inception = {
						...
						serviceConfig = {
							ExecStart = "${pkgs.nix}/bin/nix run github:NiXium-org/NiXium#nixos-tupac-unstable-install";
		...
		apps.nixos-tupac-unstable-unattended-installer-iso.program = self'.packages.nixos-tupac-unstable-unattended-installer-iso;
	};
}
