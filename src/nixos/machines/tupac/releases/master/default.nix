...
in {
	...
	perSystem = { system, pkgs, inputs', self', ... }: {
		...
		packages.nixos-tupac-master-unattended-installer-iso = inputs.nixos-generators.nixosGenerate {
			pkgs = import inputs.nixpkgs {
				inherit system;
			...
			modules = [
				{
					...
					systemd.services.inception = {
						...
						serviceConfig = {
							ExecStart = "${pkgs.nix}/bin/nix run github:NiXium-org/NiXium#nixos-tupac-master-install";
		...
		apps.nixos-tupac-master-unattended-installer-iso.program = self'.packages.nixos-tupac-master-unattended-installer-iso;
	};
}
