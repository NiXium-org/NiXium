...
###! This script performs declarative installation of NiXium-Managed NixOS UNSTABLE for the IGNUCIUS system
###!
###! For that we utilize:
...
if [ -s "$ragenixIdentity" ]; then
	...
	[ -s "/run/agenix/ignucius-disks-password" ] || age --identity "$ragenixIdentity" --decrypt --output "/run/agenix/ignucius-disks-password" "$secretPasswordPath"

	[ -s "/run/agenix/ignucius-ssh-ed25519-private" ] || age --identity "$ragenixIdentity" --decrypt --output "/run/agenix/ignucius-ssh-ed25519-private" "$secretSSHHostKeyPath"
	...
	[ -s "/run/agenix/ignucius-disks-password" ] || echo "000000" > "/run/agenix/ignucius-disks-password"

	[ -s "/run/agenix/ignucius-ssh-ed25519-private" ] || ssh-keygen -f "/run/agenix/ignucius-ssh-ed25519-private" -N ""
fi

#! Pre-build the system configuration
status "Pre-building the system configuration"
nixos-rebuild build --flake "$FLAKE_ROOT#nixos-ignucius-unstable" # pre-build the configuration

#! Perform the Payload
status "Performing the system installation"
disko-install \
	--flake "$FLAKE_ROOT#nixos-ignucius-unstable" \
	--mode format \
	--debug \
	--disk system "$(realpath "$systemDevice" || true)" \
	--extra-files "/run/agenix/ignucius-ssh-ed25519-private" /nix/persist/system/etc/ssh/ssh_host_ed25519_key
