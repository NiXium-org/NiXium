...
###! This script performs declarative installation of NiXium-Managed NixOS MASTER for the IGNUCIUS system
###!
###! For that we utilize:
...
if [ -s "$ragenixIdentity" ]; then
	...
	[ -s "/run/agenix/ignucius-disks-password" ] || echo "000000" > "/run/agenix/ignucius-ssh-ed25519-private"
...
nixos-rebuild build --flake "$FLAKE_ROOT#nixos-ignucius-master" # pre-build the configuration

#! Perform the Payload
status "Performing the system installation"
esudo disko-install \
	--flake "$FLAKE_ROOT#nixos-ignucius-master" \
	--mode format \
	--debug \
