...
# Manage Ragenix - The Rust implementation of age for managing secret files
	...
	[ -L "$ragenixTempDir" ] || {
		...
		ln -s "$ragenixTempDir.d/1" "$ragenixTempDir"
...
nixos-rebuild build --flake "$FLAKE_ROOT#nixos-tsvetan-master" # pre-build the configuration
...
esudo disko-install \
	--flake "$FLAKE_ROOT#nixos-tsvetan-master" \
	--mode format \
	--debug \
