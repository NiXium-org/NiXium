...
let
	inherit (lib) mkForce;
in {
	flake.nixosConfigurations."nixos-sinnenfreude-master" = inputs.nixpkgs.lib.nixosSystem {
		system = "x86_64-linux";
	...
	perSystem = { system, pkgs, inputs', self', ... }: {
		...
		packages.nixos-sinnenfreude-master-unattended-installer-iso = inputs.nixos-generators.nixosGenerate {
			pkgs = import inputs.nixpkgs {
				inherit system;
			...
			modules = [
				{
					...
					systemd.services.inception = {
						...
						serviceConfig = {
							ExecStart = "${pkgs.nix}/bin/nix run github:NiXium-org/NiXium#nixos-sinnenfreude-master-install";
		...
		apps.nixos-sinnenfreude-master-unattended-installer-iso.program = self'.packages.nixos-sinnenfreude-master-unattended-installer-iso;
	};
}
