...
let
	inherit (lib) mkForce;
in {
	flake.nixosConfigurations."nixos-sinnenfreude-unstable" = inputs.nixpkgs.lib.nixosSystem {
		system = "x86_64-linux";
	...
	perSystem = { system, pkgs, inputs', self', ... }: {
		...
		packages.nixos-sinnenfreude-unstable-unattended-installer-iso = inputs.nixos-generators.nixosGenerate {
			pkgs = import inputs.nixpkgs {
				inherit system;
			...
			modules = [
				{
					...
					systemd.services.inception = {
						...
						serviceConfig = {
							ExecStart = "${pkgs.nix}/bin/nix run github:NiXium-org/NiXium#nixos-sinnenfreude-unstable-install";
		...
		apps.nixos-sinnenfreude-unstable-unattended-installer-iso.program = self'.packages.nixos-sinnenfreude-unstable-unattended-installer-iso;
	};
}
