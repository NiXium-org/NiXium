...
# Module exporting configuration from NXC to other systems
...
in mkMerge [
	{
		# SSHD on Onions
		age.secrets.nxc-openssh-onion = {
			file = ../secrets/nxc-openssh-onion.age;
			...
			path = "/var/lib/tor/nxc-onion.conf";
		...
		services.tor.settings."%include" = [
			config.age.secrets."nxc-openssh-onion".path
		];
	}
]
