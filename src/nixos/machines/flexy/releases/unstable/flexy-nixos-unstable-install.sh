...
###! This script performs declarative installation of NiXium-Managed NixOS UNSTABLE for the FLEXY system
###!
###! For that we utilize:
...
nixos-rebuild build --flake "$FLAKE_ROOT#nixos-flexy-unstable" # pre-build the configuration

#! Perform the Payload
status "Performing the system installation"
disko-install \
	--flake "$FLAKE_ROOT#nixos-flexy-unstable" \
	--mode format \
	--debug \
