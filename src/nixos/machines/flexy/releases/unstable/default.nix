...
let
	inherit (lib) mkForce;
in {
	flake.nixosConfigurations."nixos-flexy-unstable" = inputs.nixpkgs.lib.nixosSystem {
		system = "x86_64-linux";
	...
	# Task to perform installation of FLEXY in NixOS distribution, unstable release
	perSystem = { system, pkgs, inputs', self', ... }: {
		packages.nixos-flexy-unstable-install = pkgs.writeShellApplication {
				name = "nixos-flexy-unstable-install";
				bashOptions = [
				...
				runtimeEnv = {
					systemDevice = self.nixosConfigurations.nixos-flexy-unstable.config.disko.devices.disk.system.device;

					secretPasswordPath = self.nixosConfigurations.nixos-flexy-unstable.config.age.secrets.flexy-disks-password.file;

					secretSSHHostKeyPath = self.nixosConfigurations.nixos-flexy-unstable.config.age.secrets.flexy-ssh-ed25519-private.file;
				};
				text = builtins.readFile ./flexy-nixos-unstable-install.sh;
			};

		# Declare for `nix run`
		apps.nixos-flexy-unstable-install.program = self'.packages.nixos-flexy-unstable-install;

		# Unattended installer
		packages.nixos-flexy-unstable-unattended-installer-iso = inputs.nixos-generators.nixosGenerate {
			pkgs = import inputs.nixpkgs {
				inherit system;
			...
			modules = [
				{
					...
					systemd.services.inception = {
						...
						serviceConfig = {
							ExecStart = "${pkgs.nix}/bin/nix run github:NiXium-org/NiXium#nixos-flexy-unstable-install";
		...
		apps.nixos-flexy-unstable-unattended-installer-iso.program = self'.packages.nixos-flexy-unstable-unattended-installer-iso;
	};
}
