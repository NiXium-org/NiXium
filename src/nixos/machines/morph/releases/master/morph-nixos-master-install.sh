...
###! This script performs declarative installation of NiXium-Managed NixOS MASTER for the MORPH system
###!
###! For that we utilize:
...
nixos-rebuild build --flake "$FLAKE_ROOT#nixos-morph-master" # pre-build the configuration

#! Perform the Payload
status "Performing the system installation"
esudo disko-install \
	--flake "$FLAKE_ROOT#nixos-morph-master" \
	--mode format \
	--debug \
