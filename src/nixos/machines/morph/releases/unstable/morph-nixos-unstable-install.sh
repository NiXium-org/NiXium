...
###! This script performs declarative installation of NiXium-Managed NixOS UNSTABLE for the MORPH system
###!
###! For that we utilize:
...
nixos-rebuild build --flake "$FLAKE_ROOT#nixos-morph-unstable" # pre-build the configuration

#! Perform the Payload
status "Performing the system installation"
esudo disko-install \
	--flake "$FLAKE_ROOT#nixos-morph-unstable" \
	--mode format \
	--debug \
