...
# Nix-based Disk Management of COOKIE

# TODO(Krey->Tanvir): Please decide on the formatting strategy for disko, note that NiXium treats all systems as throw-aways in terms of data where personal files are pending implementation of https://github.com/NiXium-org/NiXium/issues/16, if this system has files that you want to persist than that needs to be brainstormed e.g. one disk for OS and the rest will be mounted on deployment and won't be touched by disko
