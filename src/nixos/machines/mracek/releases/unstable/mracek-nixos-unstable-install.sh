...
if [ -s "$ragenixIdentity" ]; then
	...
	[ -s "/run/agenix/mracek-disks-password" ] || age --identity "$ragenixIdentity" --decrypt --output "/run/agenix/mracek-disks-password" "$secretPasswordPath"

	[ -s "/run/agenix/mracek-ssh-ed25519-private" ] || age --identity "$ragenixIdentity" --decrypt --output "/run/agenix/mracek-ssh-ed25519-private" "$secretSSHHostKeyPath"
	...
	[ -s "/run/agenix/mracek-disks-password" ] || echo "000000" > "/run/agenix/mracek-disks-password"

	[ -s "/run/agenix/mracek-ssh-ed25519-private" ] || ssh-keygen -f "/run/agenix/mracek-ssh-ed25519-private" -N ""
fi

#! Pre-build the system configuration
status "Pre-building the system configuration"
nixos-rebuild build --flake "$FLAKE_ROOT#nixos-mracek-unstable" # pre-build the configuration

#! Perform the Payload
status "Performing the system installation"
disko-install \
	--flake "$FLAKE_ROOT#nixos-mracek-unstable" \
	--mode format \
	--debug \
